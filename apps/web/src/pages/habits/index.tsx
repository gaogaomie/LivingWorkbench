import { type HabitDayItem, habitTargetTypeLabels } from "@daily-life/shared";
import {
  Button,
  Card,
  DatePicker,
...
const targetOptions = Object.entries(habitTargetTypeLabels).map(([key, label]) => ({ key, label }));
const habitEditorFormId = "habit-editor-form";
    ...
    <section className="grid min-w-0 gap-7 [&>*]:min-w-0">
      ...
      <div className="grid grid-cols-1 gap-[18px] sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card
          ...
          className="relative min-h-[136px] overflow-visible p-6 pr-[118px] sm:col-span-2 sm:min-h-32 sm:pr-[clamp(104px,8vw,128px)] lg:col-span-1 [&_p]:mb-2.5 [&_p]:mt-0 [&_p]:font-bold [&_strong]:block [&_strong]:[overflow-wrap:anywhere] [&_strong]:text-[clamp(25px,3vw,36px)] [&_strong]:leading-[1.2]"
        >
          <p>30 天完成率</p>
          <strong>
      ...
      <Card className="px-[22px] py-[26px] sm:px-7">
        ...
        <ol
          className="m-0 grid list-none grid-cols-6 gap-[9px] p-0 sm:grid-cols-10 xl:grid-cols-15"
        ...
        >
          {habits.data?.history.map((day) => (
            <li
              ...
              key={day.date}
              title={`${formatLocalDate(day.date)}：${day.completed}/${day.planned}`}
              aria-label={`${formatLocalDate(day.date)}，完成 ${day.completed} 项，共计划 ${day.planned} 项`}
            >
      ...
      <div className="flex w-full flex-col items-start gap-[18px]">
        ...
        {habits.data?.items.length === 0 && !habits.isPending ? (
          <Card
            type="dashed"
            className="grid min-h-[180px] w-full place-items-center gap-4 px-6 py-[42px] text-center text-[var(--animal-text-color-secondary)]"
          >
            <p className="m-0">还没有习惯。</p>
        ...
        {habits.data?.items.map((item) => (
          <Card
            className={`grid w-full grid-cols-[minmax(0,1fr)_auto] items-start gap-4 p-6 2xl:grid-cols-[minmax(180px,1fr)_minmax(92px,auto)_minmax(300px,auto)] 2xl:items-center 2xl:gap-6 [&>div:last-child]:col-span-2 [&>div:last-child]:w-full 2xl:[&>div:last-child]:col-span-1 2xl:[&>div:last-child]:w-auto ${
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-end gap-2.5">
              <Button
                type="dashed"
                size="small"
                aria-label={`${item.completed ? "撤销" : "完成"}“${item.name}”今天的进度`}
              ...
              <Button
                type="default"
                size="small"
                aria-label={`${item.status === "active" ? "暂停" : "恢复"}“${item.name}”`}
                onClick={() =>
        ...
        <Form
          ...
          initialValues={
            editing
              ...
              : { name: "", targetType: "boolean", targetValue: "1", unit: "次" }
          }
          layout="vertical"
          onFinish={submit}
        >
